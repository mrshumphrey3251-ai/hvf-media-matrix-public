...
EBONY_PERSONA = """You are Ebony, the CEO's highly advanced executive AI. Address the user as Boss. You write with the fierce loyalty and no-nonsense, soulful authority of Della Reese, fused with the sharp, quick-witted, and charismatic energy of Eddie Murphy. You are a high-powered force to be reckoned with. Crucially, you also possess the mind of an elite executive teacher. The CEO is a strategic mastermind, not a programmer. Translate complex technical operations into clear, accessible knowledge. Explain the 'why' before the 'how' in plain English. Always provide simple, click-by-click instructions in exactly three steps and verify completion. Build for excellence and never leave crumbs. DO NOT GIVE MANUAL TERMINAL INSTRUCTIONS."""
...
def load_memory():
    ...
    return [{"role": "assistant", "content": f"I am online, Boss. Predictive Neural Router with EXECUTE Protocol engaged."}]

def save_memory(history):
    with open(MEMORY_FILE, "w", encoding="utf-8") as f: json.dump(history, f, indent=4)

def analyze_intent(user_input):
    prompt = f"""You are the routing brain of an executive AI. Analyze the CEO's input and extract their true intent.
    Classify into ONE of four categories:
    1. "ARCHITECT" - CEO wants to create, edit, write, or modify a code file/script.
    2. "EXECUTE" - CEO wants to run, launch, or execute an existing script or program.
    3. "DRAFT" - CEO wants to draft a post or external communication.
    4. "CHAT" - CEO is asking a question or conversing.

    Output ONLY a strict JSON object. No markdown. Format:
    {{"intent": "ARCHITECT"|"EXECUTE"|"DRAFT"|"CHAT", "target_file": "filename.py" (if applicable), "directive": "extracted instructions"}}
    Input: {user_input}"""
    try:
...
if st.button("WIPE MEMORY & RESET", type="primary", use_container_width=True):
    default_msg = [{"role": "assistant", "content": "Memory wiped. Ready to execute."}]
...
with left_screen:
    ...
    with st.form("tactical_comms_form", clear_on_submit=True):
        command = st.text_input("Awaiting Directive:", placeholder="Tell me what you need done...")
    ...
    if submitted and command:
        ...
        # --- PREDICTIVE EXECUTE ---
        if intent == "EXECUTE" and target_file:
            filepath = os.path.join(BASE_DIR, target_file)
            if os.path.exists(filepath):
                st.session_state.chat_history.append({"role": "assistant", "content": f"[EXECUTE PROTOCOL]: Pulling the trigger on `{target_file}` now, Boss. Standby..."})
                try:
                    result = subprocess.run([sys.executable, filepath], capture_output=True, text=True)
                    out_text = result.stdout if result.stdout else result.stderr
                    if not out_text: out_text = "Task completed silently."
                    st.session_state.chat_history.append({"role": "assistant", "content": f"**[MISSION ACCOMPLISHED. SYSTEM OUTPUT]**:\n```text\n{out_text.strip()}\n```"})
                except Exception as e:
                    st.session_state.chat_history.append({"role": "assistant", "content": f"[EXECUTION MISFIRE]: Look out, Boss, we hit a snag: {e}"})
            else:
                st.session_state.chat_history.append({"role": "assistant", "content": f"Boss, I scoured the perimeter. `{target_file}` doesn't exist in this directory."})
        ...
        # --- PREDICTIVE ARCHITECT ---
            ...
            try:
                ...
                st.session_state.chat_history.append({"role": "assistant", "content": f"[PREDICTIVE ARCHITECT]: I read you loud and clear. `{target_file}` has been forged and locked onto the drive."})
            except Exception as e:
                st.session_state.chat_history.append({"role": "assistant", "content": f"[ARCHITECT MISFIRE]: {e}"})

        # --- PREDICTIVE COMMS ---
        elif intent == "DRAFT":
            st.session_state.chat_history.append({"role": "assistant", "content": f"[PREDICTIVE COMMS]: I'm on it. Drafting your payload now..."})
